...
static CKeyInfo gs_aKeys[] =
	{
		...
		// ssc
		{Localizable("Toggle Bot"), "toggle ssc_bot_enabled 1 0", 0, 0},
		{Localizable("Toggle Esp"), "toggle ssc_esp_enabled 1 0", 0, 0},
		{Localizable("Toggle Hidden menu"), "toggle ssc_hidden 1 0", 0, 0}
};
...
void CMenus::RenderSettings(CUIRect MainView)
{
	...
	const char *apTabs[SETTINGS_LENGTH] = {
		...
		Localize("DDNet"),
		Localize("Assets"),
		g_Config.m_ClSSClientHidden ? "Profiles" : "SSClient"
	};
	...
	else if(g_Config.m_UiSettingsPage == SETTINGS_SSCLIENT)
	{
		GameClient()->m_MenuBackground.ChangePosition(CMenuBackground::POS_SETTINGS_SSCLIENT);
		RenderSettingsSSClient(MainView);
	}
	else
	{
...
void CMenus::RenderSettingsSSClient(CUIRect MainView)
{
	static int m_SSClientSettingsTab = 1; // Default selected tab

	CUIRect Tabs, Content, Space, Spaciale;

	static float LineMargin = 20.0f;
	static float TabMargin  = 30.0f;

	static float LineMarginP = 20.0f;
	...
	MainView.HSplitTop(24.0f, &Tabs, &Content); // Top tab bar height

	// Button Containers
	static CButtonContainer s_TabAimbot, s_TabBot, s_TabTas, s_TabMisc, s_TabBinds, s_TabProfiles;
	float TabWidth = Tabs.w / 6.0f;

	// Render Menu Tabs
	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabAimbot, "Aimbot", m_SSClientSettingsTab == 1, &Space, IGraphics::CORNER_L, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 1;
	}

	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabBot, "Bot", m_SSClientSettingsTab == 2, &Space, IGraphics::CORNER_NONE, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 2;
	}

	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabTas, "Tas", m_SSClientSettingsTab == 3, &Space, IGraphics::CORNER_NONE, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 3;
	}

	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabMisc, "Misc", m_SSClientSettingsTab == 4, &Space, IGraphics::CORNER_NONE, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 4;
	}

	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabBinds, "Binds", m_SSClientSettingsTab == 5, &Space, IGraphics::CORNER_NONE, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 5;
	}

	Tabs.VSplitLeft(TabWidth, &Space, &Tabs);
	if (DoButton_MenuTab(&s_TabProfiles, "Profiles", m_SSClientSettingsTab == 6, &Space, IGraphics::CORNER_R, nullptr, nullptr, nullptr, nullptr, 10.0f))
	{
		m_SSClientSettingsTab = 6;
	}

	Content.Margin(10.0f, &Content); // Inner spacing

	if (m_SSClientSettingsTab == 1) // Aimbot Tab
	{
		Content.HSplitTop(LineMargin, &Space, &Content);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientAimbotEnabled, "Enabled", g_Config.m_ClSSClientAimbotEnabled, &Space))
			g_Config.m_ClSSClientAimbotEnabled ^= 1;

		Content.HSplitTop(LineMargin, &Space, &Content);
		Ui()->DoScrollbarOption(&g_Config.m_ClSSClientAimbotFov, &g_Config.m_ClSSClientAimbotFov, &Space, "Fov", 0, 359);
	}
	if (m_SSClientSettingsTab == 2) // Bot Tab
	{
		Content.HSplitTop(LineMargin, &Space, &Content);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientBotEnabled, "Enabled", g_Config.m_ClSSClientBotEnabled, &Space))
			g_Config.m_ClSSClientBotEnabled ^= 1;

		Content.HSplitTop(LineMargin, &Space, &Content);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientBotHookEnabled, "Hook avoid", g_Config.m_ClSSClientBotHookEnabled, &Space))
			g_Config.m_ClSSClientBotHookEnabled ^= 1;

		Content.HSplitTop(LineMargin, &Space, &Content);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientBotMoveEnabled, "Movement Avoid", g_Config.m_ClSSClientBotMoveEnabled, &Space))
			g_Config.m_ClSSClientBotMoveEnabled ^= 1;

		Content.HSplitTop(LineMargin, &Space, &Content);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientBotAimAssistEnabled, "Aim assist", g_Config.m_ClSSClientBotAimAssistEnabled, &Space))
	...
	{
		Content.HSplitTop(LineMargin+5.f, &Space, &Content);
		Ui()->DoLabel(&Space, "TAS!", 20.f, TEXTALIGN_CENTER);

		Content.HSplitTop(LineMargin, &Space, &Content);

		static CLineInput s_TasSaver(g_Config.m_ClSSClientTasPath, sizeof(g_Config.m_ClSSClientTasPath));
		s_TasSaver.SetEmptyText(Localize("tas_name"));
		Ui()->DoEditBox(&s_TasSaver, &Space, 14.0f);

		Content.HSplitTop(LineMargin*3, &Spaciale, &Content);
		
		CUIRect Left, Right;

		Spaciale.VSplitMid(&Left, &Right);

		Left.Margin(0.8f, &Left);
		Right.Margin(0.8f, &Right);

		Right.HSplitTop(LineMargin, &Space, &Right);
		static CButtonContainer s_ButtonSaveTas, s_ButtonLoadTas;;
		if(DoButton_Menu(&s_ButtonSaveTas, "Save TAS", 0, &Space))
		{
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "data/tas/%s.ssc", g_Config.m_ClSSClientTasPath);
			m_pClient->m_SSClient.SaveTas(aBuf);
			
			// Build the "saved" message:
			char aMsg[512];
			str_format(aMsg, sizeof(aMsg), "%s has been saved", aBuf);
		
			// Echo it to chat
			GameClient()->Echo(aMsg);
		}

		Left.HSplitTop(LineMargin, &Space, &Left);
		if(DoButton_Menu(&s_ButtonLoadTas, "Load TAS", 0, &Space))
		{
			char aBuf[256];
			str_format(aBuf, sizeof(aBuf), "data/tas/%s.ssc", g_Config.m_ClSSClientTasPath);
			m_pClient->m_SSClient.LoadTas(aBuf);
		
			// Build the “loaded” message:
			char aMsg[512];
			str_format(aMsg, sizeof(aMsg), "%s has been loaded", aBuf);
	...
	if (m_SSClientSettingsTab == 4) // Misc Tab
	{
		CUIRect LeftSide, RightSide;
    	Content.VSplitMid(&LeftSide, &RightSide);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoLabel(&Space, "Fake Ping", 20.f, TEXTALIGN_CENTER);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientFakePingEnabled, "Enabled", g_Config.m_ClSSClientFakePingEnabled, &Space))
			g_Config.m_ClSSClientFakePingEnabled ^= 1;

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoScrollbarOption(&g_Config.m_ClSSClientFakePing, &g_Config.m_ClSSClientFakePing, &Space, "Fake Ping Amount", 0, 1000);

		LeftSide.HSplitTop(TabMargin, &Space, &LeftSide);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoLabel(&Space, "Hide Client Tab", 20.f, TEXTALIGN_CENTER);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientHidden, "Enabled", g_Config.m_ClSSClientHidden, &Space))
			g_Config.m_ClSSClientHidden ^= 1;

		LeftSide.HSplitTop(TabMargin, &Space, &LeftSide);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoLabel(&Space, "Esp", 20.f, TEXTALIGN_CENTER);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientEspEnabled, "Enabled", g_Config.m_ClSSClientEspEnabled, &Space))
			g_Config.m_ClSSClientEspEnabled ^= 1;

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoScrollbarOption(&g_Config.m_ClSSClientEspDistance, &g_Config.m_ClSSClientEspDistance, &Space, "Max Distance", 100, 2000);
	
		LeftSide.HSplitTop(TabMargin, &Space, &LeftSide);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoLabel(&Space, "Predict path", 20.f, TEXTALIGN_CENTER);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientPredEnabled, "Enabled", g_Config.m_ClSSClientPredEnabled, &Space))
			g_Config.m_ClSSClientPredEnabled ^= 1;

		// LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		// if (DoButton_CheckBox(&g_Config.m_ClSSClientPredHookEnabled, "Predict Hook", g_Config.m_ClSSClientPredHookEnabled, &Space))
		//  	g_Config.m_ClSSClientPredHookEnabled ^= 1;

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoScrollbarOption(&g_Config.m_ClSSClientPredTicks, &g_Config.m_ClSSClientPredTicks, &Space, "Prediction ticks", 5, 300);
	
		LeftSide.HSplitTop(TabMargin, &Space, &LeftSide);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		Ui()->DoLabel(&Space, "Fake Aim", 20.f, TEXTALIGN_CENTER);

		LeftSide.HSplitTop(LineMargin, &Space, &LeftSide);
		if (DoButton_CheckBox(&g_Config.m_ClSSClientFakeAimEnabled, "Enabled", g_Config.m_ClSSClientFakeAimEnabled, &Space))
			g_Config.m_ClSSClientFakeAimEnabled ^= 1;
	}
	// binds tab
	if (m_SSClientSettingsTab == 5)
	{
		for(auto &Key : gs_aKeys)
			Key.m_KeyId = Key.m_ModifierCombination = 0;
		...
		vec2 ScrollOffset(0.0f, 0.0f);
		CScrollRegionParams ScrollParams;
		ScrollParams.m_ScrollUnit = 120.0f; // Velocidade do scroll
		s_ScrollRegion.Begin(&Content, &ScrollOffset, &ScrollParams);
		CUIRect Space_b, Left, Right;

		Content.VMargin(5.0f, &Content);
		Content.VSplitMid(&Left, &Right, 5.f);

		Left.HSplitTop(240.0f, &Space_b, &Left);
		if(s_ScrollRegion.AddRect(Space_b))
		{
			Space_b.Draw(ColorRGBA(1, 1, 1, 0.25f), IGraphics::CORNER_ALL, 10.0f);
			Space_b.VMargin(10.f, &Space_b);
			Space_b.HSplitTop(10.f, nullptr, &Space_b);

			Space_b.HSplitTop(LineMargin, &Space, &Space_b);
			Ui()->DoLabel(&Space, "Main Binds", 20.f, TEXTALIGN_CENTER);

			Space_b.HSplitTop(10.f, &Space, &Space_b);
			DoSettingsControlsButtons(44, 47, Space_b);
		}

		s_ScrollRegion.End();
	}
	if (m_SSClientSettingsTab == 6)
	{
		RenderSettingsProfiles(Content);
	}
}
